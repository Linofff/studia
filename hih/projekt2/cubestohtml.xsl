...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="currentYear" select="2026" />
  ...
  <xsl:template match="inventor">
    ...
    <p>Rok urodzenia: <xsl:value-of select="format-number(yearofbirth, '####')" />
    </p>
  ...
  <p>Główna nagroda: <xsl:value-of select="awards/award[1]/name" />
  </p>
</xsl:template>

<xsl:template match="@source">
  <a href="{.}" target="_blank" title="Idź do Wikipedii">
...
<xsl:template match="methods">
  <h2>
    Lista Metod (<xsl:value-of select="count($allMethods)" />)
  </h2>
...
<xsl:template match="tip">
  <li>
    <xsl:attribute name="style">
      <xsl:if test="position() mod 2 = 0"> color: #62afe3; </xsl:if>
      <xsl:if test="position() = last()">
          font-weight: bold; color: #63c23a;
      </xsl:if>
    </xsl:attribute>
    <xsl:apply-templates />
  </li>
...
<xsl:template match="resources">
  ...
  <ul style="list-style-type: none;">
    <xsl:apply-templates select="youtubeChannel" />
  </ul>
</xsl:template>

<xsl:template match="youtubeChannel">
  <li>
    <xsl:number format="I. " />
    <xsl:value-of select="." />
  </li>
...
<xsl:template name="method">
 ...
 Metoda:
    <xsl:value-of select="@name" />
    <xsl:if test="@creator">
        (Twórca: <xsl:value-of select="@creator" />)
    </xsl:if>
    <xsl:choose>
  ...
  <p>Rok: <xsl:value-of select="@year" />
  <span class="yearnumber"> 
        (Metoda ma <xsl:value-of select="format-number($currentYear - @year, '0.0')" />
 lat) 
  </span>
  </p>
  <p>Opis: <xsl:apply-templates select="description" /></p>
  <h4>Kroki:</h4>
  <ol>

...
>
  ...
  <xsl:template match="/">
    ...
    <html>
      ...
      <body>
        ...
        <!-- end wrapper -->
        <footer>
          <xsl:apply-templates select="hypertext/author" />	<!-- wywołanie szablonu pozwalającego na
          wyświetlenie informacji o imieniu oraz nazwisku studenta -->
        </footer>
      </body>
    </html>
  ...
  <xsl:template match="author"> Copyright 2022, <xsl:value-of select="name" /> <xsl:text> </xsl:text> <xsl:value-of
      select="surname" />
  </xsl:template>
